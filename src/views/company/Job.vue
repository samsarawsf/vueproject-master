  ...
  <div>
    ...
    <el-form :inline="true" :model="formInline" class="user-search">
      <el-form-item label="搜索：">
        <el-input size="small" v-model="formInline.name" placeholder="输入职位名称"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" style="width: 100%;">
      ...
      <el-table-column sortable prop="id" label="职位编号" width="100">
      </el-table-column>
      <el-table-column sortable prop="name" label="职位名称" width="300">
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click="closeDialog">
      <el-form label-width="120px" :model="editForm" :rules="rules" ref="editForm">
        ...
        <el-form-item label="职位名称" prop="name">
          <el-input size="small" v-model="editForm.name" auto-complete="off" placeholder="请输入职位名称"></el-input>
        </el-form-item>
        <el-form-item label="职位职责" prop="remark">
          <el-input size="small" v-model="editForm.remark" auto-complete="off" placeholder="请输入职位职责"></el-input>
        </el-form-item>
      </el-form>
...
import {
  ...
  jobSave,
  jobDelete,
  jobStatus, listEmpData, listEmpDataJob
} from '../../api/userMG'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        remark: [{ required: true, message: '请输入职位职责', trigger: 'blur' }]
      },
      formInline: {
        page: 1,
        limit: 10,
        ...
        if (valid) {
          jobSave(this.editForm)
            ...
            .catch(err => {
              this.editFormVisible = false
              this.loading = false
              this.$message.error('职位保存失败，请稍后再试！')
            })
        } else {
          return false
        }
      ...
      this.$confirm('确定要删除吗?', '信息', {
        ...
        .then(() => {
          ...
          jobDelete(parm)
            ...
            .catch(err => {
              this.loading = false
              this.$message.error('职位删除失败，请稍后再试！')
            })
        })
        .catch(() => {
          this.$message({
