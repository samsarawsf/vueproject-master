...
<template>
  <div>
    ...
    <el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" style="width: 100%;">
      ...
      <el-table-column sortable prop="name" label="角色名称" width="300">
      </el-table-column>
      <el-table-column sortable prop="roleKey" label="角色代码" width="300">
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click='closeDialog("edit")'>
      <el-form label-width="120px" :model="editForm" ref="editForm" :rules="rules">
        ...
        <el-form-item label="角色代码" prop="roleKey">
          <el-input size="small" v-model="editForm.roleKey" auto-complete="off" placeholder="请输入角色代码"></el-input>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    handleEdit: function(index, row) {
      ...
      if (row !== undefined && row !== 'undefined') {
        ...
        this.editForm.roleKey = row.roleKey
      } else {
        this.title = '添加'
        this.editForm.id = ''
        this.editForm.name = ''
        this.editForm.roleKey = ''
      }
    },
    // 编辑、增加页面保存方法
    submitForm(editData) {
      ...
      RoleRightTree(parm)
        .then(res => {
          if (res.status ===200 ) {
            ...
            this.$refs.tree.setCheckedKeys(res.data)   //设置默认选择的参数
    ...
    menuPermSave() {
      ...
      if (node.length !== 0) {
        ...
        // parm.menuIds = JSON.stringify(moduleIds)
        parm.menuIds  = moduleIds
        // console.log(parm)
      }
      RoleRightSave(parm)
        .then(res => {
    ...
    reset(){
      this.checkmenu.forEach(ele=>{
        this.$nextTick(()=>{
          this.$refs.tree.setCheckedNodes(ele.id,false,true)
        })
      })
    }
  }
